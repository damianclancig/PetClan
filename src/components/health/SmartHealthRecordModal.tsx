...
interface SmartHealthRecordModalProps {
  ...
  mode?: 'create' | 'edit'
  initialRecord?: Partial<IHealthRecord> | null
...
export function SmartHealthRecordModal({
    ...
    if (opened) {
      ...
      if (isEditMode) {
        const recordType: RecordType =
          initialRecord.type === 'deworming'
            ? initialRecord.dewormingType === 'external'
              ? 'external_deworming'
              : 'deworming'
            : initialRecord.type === 'vaccine'
              ? 'vaccine'
              : initialRecord.type === 'weight'
                ? 'weight'
                : 'consultation'
        ...
        form.setValues({
          ...
          type: (initialRecord.type || 'consultation') as any,
          appliedAt,
          nextDueAt,
          description: initialRecord.description || '',
          vetClinic: initialRecord.vetName || initialRecord.clinicName || '',
          weightValue: initialRecord.weightValue,
          vaccineType: initialRecord.vaccineType || '',
          dewormingType: initialRecord.dewormingType || 'internal',
          durationDays: initialRecord.durationDays,
        })

        if (recordType === 'external_deworming') {
          setExternalMethod('pipette')
          setExternalDuration(String(initialRecord.durationDays || 30))
          setCustomDurationDays(initialRecord.durationDays || 30)
          setExternalTargets(['fleas', 'ticks'])
        }
  ...
  }, [opened, initialConfig, mode, initialRecord])
  ...
  useEffect(() => {
    ...
    const relevantSlots = schedule.filter((slot) => {
      const isDeworming = slot.vaccineType.some(
        (t) => t.includes('desparasitacion') || t.includes('deworming'),
      )
      const isExternal = slot.vaccineType.some(
        (t) => t === 'external' || t === 'pulgas' || t === 'garrapatas' || t.includes('pipeta'),
      )
    ...
    const priorityScore = (status: string, isCore: boolean): number => {
      switch (status) {
        case 'overdue':
          return 4 + (isCore ? 1 : 0)
        case 'due_now':
          return 4 + (isCore ? 1 : 0) // Equal urgency to overdue
        case 'current_due':
          return 3 + (isCore ? 1 : 0)
        case 'due_soon':
          return 2
    ...
    if (actionable.length > 0) {
      ...
      const finalSuggestions = actionable.filter(
        (c) => priorityScore(c.status, c.slot.isCore) >= threshold,
      )
  ...
  const [externalMethod, setExternalMethod] = useState<'pipette' | 'tablet' | 'collar' | 'other'>(
    'pipette',
  )
    ...
    if (selectedType === 'external_deworming') {
      ...
      const days =
        externalDuration === 'other'
          ? typeof customDurationDays === 'number'
            ? customDurationDays
            : 30
          : daysMap[externalDuration] || 30
      ...
      const selectedTargets = externalTargets
        .map((t) => targetLabels[t as keyof typeof targetLabels])
        .join(', ')
  ...
  const handleSubmit = async (values: typeof form.values) => {
    const vetClinic = values.vetClinic?.trim()
    const payload = {
      ...values,
      petId,
  ...
  const renderMethodCard = (value: 'pipette' | 'tablet' | 'collar' | 'other', label: string) => {
    ...
    return (
      ...
      >
        ...
        <Text size="xs" fw={700} c={isSelected ? 'white' : 'dimmed'} style={{ lineHeight: 1.1 }}>
          {label}
        </Text>
      </Paper>
    )
  ...
  return (
    <Modal
      ...
      title={
        <Text size="lg" fw={700}>
          {step === 1 ? t('stepTitle') : t('stepDetails')}
        </Text>
      }
      size="lg"
      radius="lg"
          ...
          <form onSubmit={form.onSubmit(handleSubmit)}>
            <Stack gap="md">
              ...
              <Group justify="space-between" mt="xl">
                ...
                >
                  {mode === 'edit' ? t('btn.cancel') : t('btn.back')}
                </Button>
                <Button type="submit" loading={isSubmitting ?? isCreating}>
                  {mode === 'edit' ? t('btn.update') : t('btn.save')}
                </Button>
              </Group>
            </Stack>
          </form>
